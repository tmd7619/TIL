...
FROM    TB_STUDENT
WHERE   (STUDENT_ADDRESS LIKE '강원%' OR STUDENT_ADDRESS LIKE '경기도%') AND
...
--반올림하여 표시핚다.) -- NULL값 없애야함
SELECT  STUDENT_NO, 
        STUDENT_NAME,   
        ROUND(AVG(POINT),1)
FROM    TB_STUDENT
JOIN    TB_GRADE USING(STUDENT_NO)
WHERE   DEPARTMENT_NO= 059 --음악학과
...
이름과 학과 이름을 출력하는 SQL 문장을 작성하시오 못품
SELECT  CLASS_NAME,
        DEPARTMENT_NAME
FROM    TB_DEPARTMENT D
JOIN    TB_CLASS C ON (C.DEPARTMENT_NO = D.DEPARTMENT_NO)
JOIN    TB_CLASS_PROFESSOR USING(CLASS_NO)
GROUP BY CATEGORY = '예체능'
HAVING 
